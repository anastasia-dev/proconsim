...
$countRec = countRecords();
$countRec =  round($countRec/4);
selectRecords($countRec, 0);
//$text .= "\r\n\r\n";
//fwrite($fp, $text);
//fclose($fp);

function selectRecords($countRec, $count){    
...
function countRecords(){    
	...
	$sqlHelper = $connection->getSqlHelper();        
    $countRec = 0;  
    $sql = "SELECT count(*) AS countRec FROM load_prices";
    $recordset = $connection->query($sql, $limit);
    if ($recordrow = $recordset->fetch())
    {                
        $countRec = $recordrow["countRec"];        
    }
    return $countRec;     
}
