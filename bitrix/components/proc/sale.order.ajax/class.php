...
$city_full = $SxGeo->getCityFull($_SERVER['REMOTE_ADDR']);
GLOBAL $city;
$city = $city_full['city']['name_ru'];
GLOBAL $region;
$region = $city_full['region']['name_ru'];
...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function getPropertyValuesFromRequest()
	{
		...
		foreach ($this->request->getFileList() as $k => $arFileData)
		{
			if (strpos($k, "ORDER_PROP_") !== false)
			{
				...
				if (is_array($arFileData) && $orderPropId!=24)
				{
	...
	protected function getValueFromCUser($property)
	{
        ...
        }elseif($property['CODE'] === 'KPP'){
		    $rsUser = CUser::GetByID($USER->GetID());
			if ($arUser = $rsUser->Fetch())
			{
				if (!empty($arUser['UF_KPP']))
				{
					$value = $arUser['UF_KPP'];
				}
			}
        }elseif($property['CODE'] === 'OGRN'){
		    $rsUser = CUser::GetByID($USER->GetID());
			if ($arUser = $rsUser->Fetch())
			{
				if (!empty($arUser['UF_OGRN']))
				{
					$value = $arUser['UF_OGRN'];
				}
			}       
		}
...
//			$locationChanged = $locValue != $this->request->get('RECENT_DELIVERY_VALUE');
	...
	protected function saveOrder($saveToSession = false)
	{
		...
		GLOBAL $city,$region;
		$EventFields = array();
		$EventFields['ORDER_ID'] = $arResult["ORDER_ID"];
		$EventFields['USER_CITY'] = $city;
		$EventFields['USER_REGION'] = $region;
        /*
		if(CModule::IncludeModule('iblock')):
		$arSelect = Array("ID", "NAME", "IBLOCK_SECTION_ID");
		$arFilter = Array("IBLOCK_ID"=>20,  "ACTIVE"=>"Y", "NAME" => $region);
		...
		if($count != 0){
			$arFilter = Array('IBLOCK_ID'=>20, 'ID' => $sect);
			$db_list = CIBlockSection::GetList(Array(), $arFilter, true);
			while($ar_result = $db_list->GetNext())
			{
				$EventFields['MAIN_CITY'] = $ar_result['NAME'];
			}
		}else{
			$arFilter = Array('IBLOCK_ID'=>20, 'NAME' => $city);
			$db_list = CIBlockSection::GetList(Array(), $arFilter, true);
			while($ar_result = $db_list->GetNext())
			{
				$EventFields['MAIN_CITY'] = $ar_result['NAME'];
        ...
        */
        ...
        if(!empty($_SESSION['REGION_NAME'])){
            $EventFields['MAIN_CITY'] = $_SESSION['REGION_NAME'];
        }else{
            $EventFields['MAIN_CITY'] = "Москва";
        }
		if(CModule::IncludeModule('iblock')):
			...
			$EventFields['POST_TEXT'] .= "Заказ №".$arResult['ORDER_ID'];
			$arFirst = array();$arSecond = array();
			$arFilter = Array("ID" => $arResult['ORDER_ID']);
			...
			while ($arSales = $rsSales->Fetch())
			{
				...
				array("SORT" => "ASC"),
				array("PERSON_TYPE_ID" => $arSales["PERSON_TYPE_ID"]));
				while ($arProps = $db_props->Fetch())
				{
					$db_vals = CSaleOrderPropsValue::GetList(array("SORT" => "ASC"),
					array("ORDER_ID" => $arResult['ORDER_ID'],"ORDER_PROPS_ID" => $arProps["ID"],"!CODE" => "LOCATION"));
					while ($arVals = $db_vals->Fetch()){
						$EventFields['POST_TEXT'] .= "\n".$arVals['NAME']." ".$arVals['VALUE'];
					}
				}
			}
			$EventFields['POST_TEXT'] .= "\nКомментарий: ".$arFirst['USER_DESCRIPTION'];
            $rsUser = CUser::GetByID($USER->GetID()); 
			$arUser = $rsUser->Fetch(); 
            if(!empty($arUser["UF_USER_CODE"])){
                $EventFields['POST_TEXT'] .= "\nЕсть доступ в ЛК";
            }else{
                $EventFields['POST_TEXT'] .= "\nНе имеет доступ в ЛК";
            }
			$EventFields['POST_TEXT'] .= "\nСумма заказа составила ".$arFirst['PRICE']." ".$arFirst['CURRENCY'];
			$EventFields['POST_TEXT'] .= "\n\nСостав заказа:";
			$dbBasketItems = CSaleBasket::GetList(
				array("NAME" => "ASC","ID" => "ASC"),array("LID" => SITE_ID,"ORDER_ID" => $arResult['ORDER_ID']),
				false,false,array("*"));
			$count_b = 1;
			$fp = fopen($_SERVER['DOCUMENT_ROOT']."/upload/order/order_".$arResult['ORDER_ID'].".txt", "w");
            
            // Менеджер
            $USER_MANAGER_EMAIL = "info@proconsim.ru";
            if(!empty($arUser["UF_MANAGER_ID"])){
                ...
                $USER_MANAGER_EMAIL = $arManager["EMAIL"];
            }
            $EventFields['MANAGER_EMAIL'] = $USER_MANAGER_EMAIL;
			$USER_ASSISTANT_CODE = $arUser['UF_ASSISTANT_CODE'];
			while ($arItems = $dbBasketItems->Fetch())
			{
				...
				while($ob_CI = $res_CI->GetNextElement())
				{
					...
					$text = "~".$arFields_CI['PROPERTY_ARTNUMBER_VALUE']."~|~".$arItems['QUANTITY']."~";
					if(!empty($USER_ASSISTANT_CODE)){
						$text .= "|~".$USER_ASSISTANT_CODE."~";
					}
					$text .= "\r\n";
					fwrite($fp, $text);
				}
				$EventFields['POST_TEXT'] .= "\nВ количестве ".$arItems['QUANTITY']." шт.";
				$EventFields['POST_TEXT'] .= "\nЦена - ".$arItems['PRICE']." ".$arItems['CURRENCY'].". Указана ".$arItems['NOTES'];
				$EventFields['POST_TEXT'] .= "\n\n";
				$count_b ++;
			}
            ...
            $arFileFields = array(
               ...
               "NAME" => "Файл",
               "CODE" => "orderFile",
               "VALUE" => "/upload/order/order_".$arResult['ORDER_ID'].".txt"
            );
            
            CSaleOrderPropsValue::Add($arFileFields);
		/*
			$propFileID = 0;
            $obProps = Bitrix\Sale\Internals\OrderPropsValueTable::getList(array('filter' => array('ORDER_ID' => $arResult['ORDER_ID'], 'ORDER_PROPS_ID' => 24)));
            ...
            if(!empty($propFileID)){
                CSaleOrderPropsValue::Update($propFileID,$arFileFields);
            }
            */
		endif;
		CEvent::Send("SALE_INFORMATION", 's1', $EventFields,"Y",47,array($_SERVER['DOCUMENT_ROOT']."/upload/order/order_".$arResult['ORDER_ID'].".txt"));
