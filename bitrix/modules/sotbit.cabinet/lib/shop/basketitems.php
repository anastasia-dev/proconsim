 ...
 */
class BasketItems extends \SotbitCabinet
{
	/**
	 * @var array
	 */
			...
			)
	{
		...
		if ($this->checkInstalledModules(array(
				...
				'currency',
				'sale'
		)) && $arFilter && !\SotbitCabinet::getInstance()->isDemo())
		{
	...
	{
		if(
				Loader::includeModule('sotbit.missshop') ||
				Loader::includeModule('sotbit.mistershop') ||
				Loader::includeModule('sotbit.b2bshop')
				)
		{
	...
	{
		$module = 'sotbit.missshop';
		if(Loader::includeModule('sotbit.mistershop'))
		{
			$module = 'sotbit.mistershop';
		}
			...
			if($offers)
			{
				...
				if($ids)
				{
					foreach($ids as $id)
					{
						...
						{
							$noPhoto = \CFile::getList(array(),array('MODULE_ID' =>'sotbit.cabinet'))->fetch();
							if(!$noPhoto['ID'])
							{
								$noPhoto = \CFile::MakeFileArray($imgProp['noPhoto']);
								$noPhoto['MODULE_ID'] = 'sotbit.cabinet';
								$imgNoPhoto = \CFile::SaveFile($noPhoto,'sotbit.cabinet');
