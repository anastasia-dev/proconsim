...
class Offers
{
	...
	protected function getIblockOffers($iblockId)
	{
		...
		if(
			...
			if(!empty($offersId)) {
				//$propertyList = $this->getPropertyList($catalog['IBLOCK_ID'],$propertyCode);
				...
				if(!empty($propertyCode)) {
					...
					foreach($offers as &$row) {
						...
						foreach($propertyCode as $pid) {
							...
							//TODO: Обязательно нужно для каталожных условий USE_CONDITIONS по значению свойств
							//(((isset($arItem['PROPERTY_520_VALUE']) && in_array(113, $arItem['PROPERTY_520_VALUE']))))
							if($dispProp = $row['PROPERTIES'][ $pid ]) {

								//TODO: Фикс для свойств типа список
								...
								$row[ 'PROPERTY_' . $dispProp['ID'] . '_VALUE' ] = $dispProp['VALUE'];
							}
