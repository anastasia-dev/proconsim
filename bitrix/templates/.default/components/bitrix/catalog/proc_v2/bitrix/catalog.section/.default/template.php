    ...
    function compare_tov(id)
    {
        ...
        if (chek.checked)
        {
            ...
            $.get("/ajax/compare.php",
                ...
                function(data) {
                    $("label[for=compareid_"+id).html('Убрать из сравнения <i class="compare_icon"></i>');
        ...
        else
        {
            ...
            $.get("/ajax/compare.php",
                ...
                function(data) {
                    $("label[for=compareid_"+id).html('Сравнение <i class="compare_icon"></i>');
                    $("#compare_list_count").html(data);
                }
            );
        }
...
<div class="goods-wrap">
    <?foreach($arResult['ITEMS'] as $arItem):?>
        ...
        <div class="goods-item">
            ...
            <div class="g-img"><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><img id="img-<?=$arItem['ID']?>" src="<?=$file['src']?>" alt="<?=$arItem['NAME']?>"></a></div>
                ...
                <?if ($arItem['CAN_BUY']):?>
                    ...
                    <div class="g-price price-product-<?=$arItem['ID']?>">
                        <?if($arItem['MIN_PRICE']['VALUE']>0){?>
                            <span nowrap><?=number_format($arItem['MIN_PRICE']['VALUE'],2,',',' ');?></span>
                        <?}else{?>
                    ...
                    <div class="comp_block">
                        <input <?=$checked;?> type="checkbox" style="display: none;" id="compareid_<?=$arItem['ID'];?>" onchange="compare_tov(<?=$arItem['ID'];?>);">
                        <label style="cursor: pointer;" for="compareid_<?=$arItem['ID'];?>"><?if ($checked) {?>Убрать из сравнения <i class="compare_icon"></i><?} else {?>Сравнение <i class="compare_icon"></i><?}?></label>
                    </div>
                <?else:?>
