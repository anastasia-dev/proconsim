...
$dbBasketItems = CSaleBasket::GetList(
    ...
    false,false,array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "PRODUCT_PROVIDER_CLASS"));
...
$rsStock = CIBlockElement::GetList(
                                   array(), 
                                   array(
                                   "IBLOCK_ID" => 19, 
                                   array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 19, "PROPERTY_PRODUCTS" => $arResult["ID"])))
                                   ),
                                   false, 
                                   false,
...
while($arStock = $rsStock->GetNext()) {
    $arResult["STOCK"][$arStock["ID"]] = array("NAME" => $arStock["NAME"], "DETAIL_PAGE_URL" => $arStock["DETAIL_PAGE_URL"]);
}

// Новинки
        $arResult["NOVELTY"] = array();
        $rsNovelty = CIBlockElement::GetList(
                                           array(), 
                                           array(
                                           "IBLOCK_ID" => 22, 
                                           array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 22, "PROPERTY_PRODUCTS" => $arResult["ID"])))
                                           ),
                                           false, 
                                           false,
                                           array("ID", "NAME", "DETAIL_PAGE_URL")
        ...
        while($arNovelty = $rsNovelty->GetNext()) {
            $arResult["NOVELTY"][$arNovelty["ID"]] = array("NAME" => $arNovelty["NAME"], "DETAIL_PAGE_URL" => $arNovelty["DETAIL_PAGE_URL"]);
        }
...
// Доставка из Москвы
   GLOBAL $CITY;
   $arSelectRegions = Array("IBLOCK_ID","ID","PROPERTY_DELIVERY_FROM_MOSCOW");
	$arFilterRegions = Array("IBLOCK_ID"=>6, "ID"=>$CITY);
	$resRegions = CIBlockElement::GetList(Array(), $arFilterRegions, false, false, $arSelectRegions);
    $dayDeclension = new Declension('день', 'дня', 'дней');
    $arResult['DELIVERY_FROM_MOSCOW'] = "";
    $arResult['DELIVERY_FROM_MOSCOW_TEXT'] = "";
	while($obRegions = $resRegions->GetNextElement())
	{
         $regRegions = $obRegions->GetFields();
		 if(!empty($regRegions["PROPERTY_DELIVERY_FROM_MOSCOW_VALUE"])){
         	$arResult['DELIVERY_FROM_MOSCOW'] = $regRegions["PROPERTY_DELIVERY_FROM_MOSCOW_VALUE"];
            $arResult['DELIVERY_FROM_MOSCOW_TEXT'] = $regRegions["PROPERTY_DELIVERY_FROM_MOSCOW_VALUE"]." ".$dayDeclension->get($regRegions["PROPERTY_DELIVERY_FROM_MOSCOW_VALUE"]);
		 }
    }

// meta
$arResult['CUSTOM_META'] = array();
  $arResult['CUSTOM_META']["TITLE"] = $arResult["PROPERTIES"]["TITLE"]["VALUE"];
  $arResult['CUSTOM_META']["KEYWORDS"] = $arResult["PROPERTIES"]["KEYWORDS"]["VALUE"];
  $arResult['CUSTOM_META']["DESCRIPTION"] = $arResult["PROPERTIES"]["META_DESCRIPTION"]["VALUE"];
...
if ($arResult['SECTION']['ID'] > 0) {
    ...
    if (is_array($templates) && !empty($templates)) {
        ...
        foreach ($templates as $templateKey => $template) {
			...
			$arResult['CUSTOM_IPROPERTY_VALUES'][$templateKey] = 
			Iblock\Template\Engine::process(
			$entity,
			str_replace("{=parent.Name}", $arResult['SECTION']['NAME'], $template['TEMPLATE'])
			 );
