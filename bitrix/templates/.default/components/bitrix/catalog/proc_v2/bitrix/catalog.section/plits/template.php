...
?>
<script>
    function rollBackCompareButtons(id) {
        $(".compare_counter a").attr("href", "#");
        $("label[for^=compareid_").html('<span class="cmarker" compared="0">Сравнить</span> <i class="compare_icon"></i>');
        $('.content input[type=checkbox]:not([id=compareid_'+id+']').prop('checked', false);

        $("label[for=compareid_"+id).html('<span class="cmarker" compared="1">К сравнению</span> <i class="compare_icon"></i>');
    ...
    function compare_tov(id)
    {
        ...
        if (chek.checked )
        {
            //Добавить
            var AddedGoodId = id;
            var clear_compare_list = $('.clear_compare_list').attr('clear_compare_list');



            if (clear_compare_list == 'y') {
                $.get("/ajax/compare.php",
                    {
                        action: "ADD_TO_COMPARE_LIST", id: AddedGoodId, 'clear_compare_list' : clear_compare_list
                    },
                    function(data) {
                        $("label[for=compareid_"+id).html('<span class="cmarker" compared="1">К сравнению</span> <i class="compare_icon"></i>');
                        $("#compare_list_count").html(data);
                        let compare_count = parseInt($(".compare-items tbody tr").length);
                        if (!compare_count) {
                            $(".compare_counter a").attr("href", "#");
                        } else {
                ...
                $.ajax({
                    type: "POST",
                    url: "/ajax/check_compare_list.php",
                    ...
                    success: function(a){
                        if (a.result == 'fail') {
                            $.magnificPopup.open({
                                items: {
                                    src: a.html, // can be a HTML string, jQuery object, or CSS selector
                                    type: 'inline'
                                }
                            });
                        } else {
        ...
        <div class="col-sm-4 col-xs-6">
            <div class="product-card">
                <div class="top-info-panel">
                    ...
                    ?>
                    <a href="<?=$arItem['DETAIL_PAGE_URL']?>"><img id="img-<?=$arItem['ID']?>" src="<?=$file["src"]?>" title="<?=$arItem['NAME']?>" alt="Купить <?=$arItem['NAME']?>"></a>
                ...
                <div class="bottom-info-panel">
                    ...
                    <?if(empty($arItem['OFFERS'])):?>
                        <?if ($arItem['CAN_BUY']):?>
                            ...
                            <?if($arItem['MIN_PRICE']['VALUE']>0){?>
                                <div class="price-on-table">
                                    ...
                                    <div class="price-product-<?=$arItem['ID']?>" id="price-product-<?=$arItem['ID']?>"><?=number_format($arItem['MIN_PRICE']['VALUE'],2,',',' ');?> <span>руб.</span></div>
                                </div>
                            <?}else{?>
                            ...
                            <div class="comp_block">
                                <input <?=$checked;?> type="checkbox" style="display: none;" data-compare="0" id="compareid_<?=$arItem['ID'];?>" onchange="compare_tov(<?=$arItem['ID'];?>);">
                                <label style="cursor: pointer;" for="compareid_<?=$arItem['ID'];?>"><?if ($checked) {?>
                                        <span>К сравнению</span> <i class="compare_icon"></i><?} else {?><span>Сравнить</span> <i class="compare_icon"></i><?}?></label>
                            </div>
                            <div class="row" style="display: flex;align-items: center;flex-wrap: wrap;">
                                ...
                                <div class="col-sm-6 col-xs-12">
                                    ...
                                    <button class="btn-bye-large b-g-s-<?=$arItem['ID']?>" title="В корзину" onclick="addToBasket('<?echo $arItem['ADD_URL'];?>','<?=$arItem['ID']?>');">КУПИТЬ<?if(intval($arResult['BASKET'][$arItem['ID']]) != 0):?><span class="in-basket-count"><?=intval($arResult['BASKET'][$arItem['ID']])?></span><?endif;?></button>
                                </div>
                            </div>
                        ...
                        <?else:?>
                            <div class="comp_block">
                                <input <?=$checked;?> type="checkbox" style="display: none;" data-compare="0" id="compareid_<?=$arItem['ID'];?>" onchange="compare_tov(<?=$arItem['ID'];?>);">
                                <label style="cursor: pointer;" for="compareid_<?=$arItem['ID'];?>"><?if ($checked) {?><span>К сравнению</span> <i class="compare_icon"></i><?} else {?><span>Сравнить</span> <i class="compare_icon"></i><?}?></label>
                            </div>
                            <div class="row">
                                <div class="article">
                                    Арт. <?=$arItem['PROPERTIES']['ARTNUMBER']['VALUE']?>
                        ...
                        <?if($canbuy):?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-7">
                                    <div class="price-on-table">
                                        <div class="price_word">Цена:</div>
                                        <div class=" price-product-<?=$id?>" id="price-product-<?=$id?>"><?=number_format($price,2,',',' ');?> <span>руб.</span></div>
                                    </div>
                                </div>
                                <div class="col-xs-5">
                                    <div class="large-wrap-count">
                                    ...
                                    <button class="btn-bye-large b-g-s-<?=$id?>" title="В корзину" onclick="addToBasket('<?echo $addUrl;?>','<?=$id;?>');">КУПИТЬ<?if(intval($arResult['BASKET'][$arItem['ID']]) != 0):?><span class="in-basket-count"><?=intval($arResult['BASKET'][$arItem['ID']])?></span><?endif;?></button>
                                </div>
                            </div>
