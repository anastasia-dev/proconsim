...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
        ...
        $rsStock = CIBlockElement::GetList(
                                           ...
                                           array(
                                           "IBLOCK_ID" => 19, 
                                           array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 19, "PROPERTY_PRODUCTS" => $arItem["ID"])))
                                           ),
                                           false, 
                                           false,
        ...
        while($arStock = $rsStock->GetNext()) {
            $arItem["STOCK"][$arStock["ID"]] = array("NAME" => $arStock["NAME"], "DETAIL_PAGE_URL" => $arStock["DETAIL_PAGE_URL"]);
        }

		// Новинки
        $arItem["NOVELTY"] = array();
        $rsNovelty = CIBlockElement::GetList(
                                           array(), 
                                           array(
                                           "IBLOCK_ID" => 22, 
                                           array("ID" => CIBlockElement::SubQuery("ID", array("IBLOCK_ID" => 22, "PROPERTY_PRODUCTS" => $arItem["ID"])))
                                           ),
                                           false, 
                                           false,
                                           array("ID", "NAME", "DETAIL_PAGE_URL")
        ...
        while($arNovelty = $rsNovelty->GetNext()) {
            $arItem["NOVELTY"][$arNovelty["ID"]] = array("NAME" => $arNovelty["NAME"], "DETAIL_PAGE_URL" => $arNovelty["DETAIL_PAGE_URL"]);
        }

        //Пути к разделам
		//if($arItem['IBLOCK_SECTION_ID']!==$arItem['~IBLOCK_SECTION_ID']){
		//$arItem["DETAIL_PAGE_URL"] = $arParams["SECTION_URL"].$arItem["CODE"]."/";
		//}
...
while($ob = $res->GetNextElement())
{
	...
	$arResult['BRANDS_LIST'][$arFields['ID']] = CFile::GetPath($arFields['PREVIEW_PICTURE']);
}
